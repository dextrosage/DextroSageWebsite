...
export const AdminMemberSessions: React.FC = () => {
  ...
  const fetchSessionData = useCallback(async (quiet = false) => {
    ...
    try {
      // 1. Fetch sessions
      const sessionList = await adminApi.getMemberSessions(userId);
      setSessions(sessionList);

      // 2. Fetch member list to lookup user name
      const memberList = await adminApi.getMembers();
  ...
  const handleDeleteSessionConfirm = async () => {
    ...
    try {
      await adminApi.deleteSession(sessionToDelete);
  ...
  const handleDeleteAllConfirm = async () => {
    ...
    try {
      await adminApi.deleteAllSessions(userId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <button
            ...
            className="inline-flex items-center text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors uppercase tracking-wide mb-2 focus:outline-none"
          >
          ...
          <p className="text-xs text-gray-500 mt-1">
            Displaying all active device logins for <strong className="text-gray-700">{memberName || userId}</strong>
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white border border-gray-100 rounded-xl p-8 text-center shadow-sm">
      ...
      >
        ...
        {sessionToDelete && (
          <div className="mt-3 p-2 bg-gray-50 border border-gray-100 rounded font-mono text-xxs text-gray-500 break-all">
      ...
      >
        <p className="text-sm text-gray-600">
          Are you sure you want to terminate <strong className="text-gray-900">every single active login session</strong> for <strong className="text-gray-900">{memberName || 'this user'}</strong>?
